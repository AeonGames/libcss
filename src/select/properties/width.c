		...
		css_computed_style *style)
{
	return set_width(style, hint->status,
		(css_fixed_or_calc) {.value = hint->data.length.value}, hint->data.length.unit);
}

css_error css__initial_width(css_select_state *state)
{
	return set_width(state->computed, CSS_WIDTH_AUTO, (css_fixed_or_calc) { .value = 0, .calc = NULL }, CSS_UNIT_PX);
}
...
{
	css_fixed_or_calc length = (css_fixed_or_calc){ .value = 0, .calc = NULL };
